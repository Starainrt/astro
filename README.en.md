...
This package supports conversion between Gregorian dates and traditional Chinese lunar dates, and also provides solar term information.
Although the traditional Chinese calendar is essentially a lunisolar calendar, for the sake of common usage and code simplicity, the relevant functions are named using Lunar instead of the more academic Lunisolar.
...
- **[1913, 3000]**: Calculated using the VSOP87 theory for determining solar terms (定气) and the ELP2000 theory for determining new moons (定朔), following the algorithm specified in the current China national standard `GB/T 33661-2017` for the Chinese lunar calendar. Both solar term and new moon calculations use Beijing Time (UTC+08:00).
- Solar terms before 1912 CE are computed using modern astronomical methods (VSOP87) and may differ by 1–2 days from the dates recorded in historical almanacs. This will be improved in future updates.
...
##### 4. Time Zone Explanation

This package is primarily designed for the Chinese calendar, so the calculations of solar terms (定气) and new moons (定朔) default to Beijing Time (UTC+8).
For regions using other time zones, directly applying the Chinese lunar calendar rules may lead to date deviations.

To facilitate exploration and research, this package provides low-level methods `Solar` and `Lunar`, which support conversion between Gregorian and lunar dates under **custom time zones** according to the **current Chinese lunar calendar algorithm (GB/T 33661-2017)**.  
If you only need standard conversions under Beijing Time, please use the ready-to-use methods `SolarToLunar` and `LunarToSolar`.

**Example**: According to lunar rules, the winter solstice must fall in the 11th lunar month. Taking the winter solstice of 1984 as an example:

```go
ws := calendar.JieQi(1984, 270)
fmt.Println(ws) // 1984-12-22 00:22:48.471944332 +0800 CST  // Winter solstice; for UTC +7 local time, it is one hour earlier, i.e., December 21
fmt.Println(moon.ClosestShuoYue(ws)) // 1984-12-22 19:46:34.422152638 +0800 CST // The new moon closest to the winter solstice
```

| Solar Term / New Moon | UTC +8 |  UTC +7 |
|-----------------------|------------------------|------------------------|
| Winter Solstice       | 1984-12-22             | 1984-12-21             |
| New Moon              | 1984-12-22             | 1984-12-22             |

As shown, for the East 8th zone (China), December 22, 1984, is both the winter solstice and the new moon, so that day is the first day of the 11th lunar month. In the East 7th zone, however, the winter solstice falls on December 21, making December 22 already the first day of the 12th lunar month.

Similarly, the date of the Chinese New Year (chunjie) can differ by one day. For the first day of the first lunar month in 1985, the corresponding Gregorian date is February 20 in the East 8th zone, but January 21 in the East 7th zone:

```go
fmt.Println(calendar.Solar(1985, 1, 1, false, 8.0)) // 1985-02-20 00:00:00 +0800 CST  (East 8th zone Chinese New Year)
fmt.Println(calendar.Solar(1985, 1, 1, false, 7.0)) // 1985-01-21 00:00:00 +0700      (East 7th zone Chinese New Year)
```

##### 5. Go Standard Library Compatibility
...
func main() {
   ...
   modernDate, _ := calendar.LunarToSolarByYMD(2026, 1, 1, false)  //The 2026 Chinese New Year（Chunjie）
...
2026-02-17 00:00:00 +0800 CST //The 2026 Chinese New Year（Chunjie）
```
...
1. This package includes a built-in database of 9,100 stars and can automatically calculate proper motion.
...
func main() {
	cst := time.FixedZone("CST", 8*3600)
	// Specify 2020-01-01 08:08:08
	date := time.Date(2020, 1, 1, 8, 8, 8, 8, cst)

    // Initialize star database
    star.InitStarDatabase()
    sirius, _ := star.StarDataByName("天狼")
    ra, dec := sirius.RaDecByDate(date) // Instantaneous RA/Dec corrected for precession, nutation, and proper motion
    // Sirius rise time
	...
	// Right ascension of Vega in the year 13600 CE
    fmt.Println(tools.Format(ra/15, 1))
	// Declination of Vega in the year 13600 CE
...
Output:
```
2019-12-31 19:22:56.176710426 +0800 CST  // Sirius rise time
2020-01-01 05:30:39.834894239 +0800 CST  // Sirius set time
5h58m10.19s // Right ascension of Vega in 13600 CE
84°19′26.25″ // Declination of Vega in 13600 CE, already very close to the north celestial pole, becoming the new North Star
```
