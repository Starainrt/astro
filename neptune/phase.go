...
func PhaseAngleN(date time.Time, n int) float64 {
	return basic.NeptunePhaseAngleN(phaseJD(date), n)
}
...
func IlluminatedFractionN(date time.Time, n int) float64 {
	return basic.NeptuneIlluminatedFractionN(phaseJD(date), n)
}
...
func BrightLimbPositionAngleN(date time.Time, n int) float64 {
	return basic.NeptuneBrightLimbPositionAngleN(phaseJD(date), n)
}
