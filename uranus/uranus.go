...
func riseSetResult(date time.Time, jde float64, err error) (time.Time, error) {
	if err != nil {
		...
		case errors.Is(err, basic.ErrNeverRise):
			return time.Time{}, ERR_URANUS_NEVER_RISE
		case errors.Is(err, basic.ErrNeverSet):
			return time.Time{}, ERR_URANUS_NEVER_SET
		default:
...
//
// 返回天王星在 date 对应绝对时刻的瞬时视黄经，单位度。
// Returns the apparent ecliptic longitude of Uranus at the instant represented by date, in degrees.
...
//
// 返回天王星在 date 对应绝对时刻的瞬时视黄纬，单位度。
// Returns the apparent ecliptic latitude of Uranus at the instant represented by date, in degrees.
...
//
// 返回天王星在 date 对应绝对时刻的瞬时视赤经，单位度。
// Returns the apparent right ascension of Uranus at the instant represented by date, in degrees.
...
//
// 返回天王星在 date 对应绝对时刻的瞬时视赤纬，单位度。
// Returns the apparent declination of Uranus at the instant represented by date, in degrees.
...
//
// 返回天王星在 date 对应绝对时刻的瞬时视赤经与视赤纬，单位度。
// Returns the apparent right ascension and declination of Uranus at the instant represented by date, in degrees.
...
//
// 返回天王星在 date 对应绝对时刻的视星等。
// Returns the apparent visual magnitude of Uranus at the instant represented by date.
...
//
// 返回天王星在 date 对应绝对时刻到地球的距离，单位 AU。
// Returns the distance from Uranus to Earth at the instant represented by date, in astronomical units.
...
//
// 返回天王星在 date 对应绝对时刻到太阳的距离，单位 AU。
// Returns the distance from Uranus to the Sun at the instant represented by date, in astronomical units.
...
func RiseTime(date time.Time, lon, lat, height float64, aero bool) (time.Time, error) {
	...
	_, loc := date.Zone()
	timezone := float64(loc) / 3600.0
	riseJde, err := basic.UranusRiseTime(jde, lon, lat, timezone, aeroFloat, height)
...
func SetTime(date time.Time, lon, lat, height float64, aero bool) (time.Time, error) {
	...
	_, loc := date.Zone()
	timezone := float64(loc) / 3600.0
	riseJde, err := basic.UranusSetTime(jde, lon, lat, timezone, aeroFloat, height)
	return riseSetResult(date, riseJde, err)
}
