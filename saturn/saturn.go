...
var (
	ERR_SATURN_NEVER_RISE = errors.New("ERROR:极夜，土星今日永远在地平线下！")
	ERR_SATURN_NEVER_SET  = errors.New("ERROR:极昼，土星今日永远在地平线上！")
...
func riseSetResult(date time.Time, jde float64, err error) (time.Time, error) {
	if err != nil {
		...
		case errors.Is(err, basic.ErrNeverRise):
			return time.Time{}, ERR_SATURN_NEVER_RISE
		case errors.Is(err, basic.ErrNeverSet):
			return time.Time{}, ERR_SATURN_NEVER_SET
		default:
...
//
// 返回土星在 date 对应绝对时刻的瞬时视黄经，单位度。
// Returns the apparent ecliptic longitude of Saturn at the instant represented by date, in degrees.
...
//
// 返回土星在 date 对应绝对时刻的瞬时视黄纬，单位度。
// Returns the apparent ecliptic latitude of Saturn at the instant represented by date, in degrees.
...
//
// 返回土星在 date 对应绝对时刻的瞬时视赤经，单位度。
// Returns the apparent right ascension of Saturn at the instant represented by date, in degrees.
...
//
// 返回土星在 date 对应绝对时刻的瞬时视赤纬，单位度。
// Returns the apparent declination of Saturn at the instant represented by date, in degrees.
...
//
// 返回土星在 date 对应绝对时刻的瞬时视赤经与视赤纬，单位度。
// Returns the apparent right ascension and declination of Saturn at the instant represented by date, in degrees.
...
//
// 返回土星在 date 对应绝对时刻的视星等。
// Returns the apparent visual magnitude of Saturn at the instant represented by date.
...
//
// 返回土星在 date 对应绝对时刻到地球的距离，单位 AU。
// Returns the distance from Saturn to Earth at the instant represented by date, in astronomical units.
...
//
// 返回土星在 date 对应绝对时刻到太阳的距离，单位 AU。
// Returns the distance from Saturn to the Sun at the instant represented by date, in astronomical units.
...
func RiseTime(date time.Time, lon, lat, height float64, aero bool) (time.Time, error) {
	...
	_, loc := date.Zone()
	timezone := float64(loc) / 3600.0
	riseJde, err := basic.SaturnRiseTime(jde, lon, lat, timezone, aeroFloat, height)
...
func SetTime(date time.Time, lon, lat, height float64, aero bool) (time.Time, error) {
	...
	_, loc := date.Zone()
	timezone := float64(loc) / 3600.0
	riseJde, err := basic.SaturnSetTime(jde, lon, lat, timezone, aeroFloat, height)
	return riseSetResult(date, riseJde, err)
}
